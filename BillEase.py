...
def gen_bill():   # generates the bill and displays in a neat format
    ...
    if state.get() == 1:
        ...
        elif float(discount.get()) > 100:
            messagebox.showerror("Error", "Discount cannot be greater than 100%")
...
def send_bill():   # sends the bill to the customer by email
    ...
    server.starttls()
    server.login('billease3@gmail.com', 'kdncpiwspprneotd')
    server.sendmail('billease3@gmail.com', to_email, msg.as_string())
    server.quit()
